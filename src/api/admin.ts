...
function queryString(query: PageQuery): string {
  ...
  if (query.establishmentStatus) params.set('establishmentStatus', query.establishmentStatus);
...
export async function countOpenVerificationCases(): Promise<number> {
  return (await listVerificationCases('OPEN')).length;
}

export async function countEstablishments(): Promise<number> {
  return (await listEstablishments()).length;
}
